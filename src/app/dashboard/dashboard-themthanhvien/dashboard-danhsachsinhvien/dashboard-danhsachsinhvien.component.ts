...
export class DashboardDanhsachsinhvienComponent implements OnInit {
   ...
   lmForm: Form = new Form({
      loiNhan: ['Bạn có muốn tham gia vào nhóm của mình không ?', Validators.required],
   });
   ...
   async ngOnInit() {
      ...
      this.lstLoiMoi = await this.loiMoiService.getAllLoiMoiSinhVienByDotNamHocNhom(
   ...
   async onSendInvite() {
      ...
      if (await this.checkGroupMemberSentInvitation()) {
         create?.classList.remove('active');
         createBox?.classList.remove('active');
         this.toastService.info('Nhóm bạn đã gửi lời mời cho sinh viên này', 'Thông báo !');
         return;
      } else {
   ...
   async createGroup() {
      var nhom = new Nhom();
      nhom.maNhom = DashboardComponent.maSV + shareService.namHoc + shareService.dot;
      nhom.tenNhom = (await this.sinhVienService.getById(DashboardComponent.maSV)).tenSv;
   ...
   async checkGroupMemberSentInvitation() {
      let lstLoiMoi = await this.loiMoiService.getAllLoiMoiSinhVienByIdDotNamHocNhom(
         this.groupIdCreated,
         this.sinhVien.maSv,
         shareService.namHoc,
         shareService.dot
   ...
   checkSentInvitationForStudent() {
      return this.lstLoiMoi.find((lm) => lm.maSv == this.sinhVien.maSv) ? true : false;
   }
