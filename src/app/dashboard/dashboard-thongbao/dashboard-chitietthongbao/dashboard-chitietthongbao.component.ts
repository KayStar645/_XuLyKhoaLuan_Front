...
@Component({
  selector: 'app-dashboard-chitietthongbao',
  templateUrl: './dashboard-chitietthongbao.component.html',
  styleUrls: [
    './dashboard-chitietthongbao.component.scss',
    '../dashboard-thongbao.component.scss',
  ],
})
export class DashboardChitietthongbaoComponent implements OnInit {
  maTb: number = -1;
  listTB: any = [];
  oldForm: any;
  pdfSrc: any;
  ...
  async onAdd() {
    if (this.tbForm.form.valid) {
      ...
      try {
        ...
        this.router.navigate(['/dashboard/thong-bao/chi-tiet', { maTb: -1 }]);
  ...
  onDelete() {
    ...
    option.agree(async () => {
      try {
        ...
        this.router.navigate(['/dashboard/thong-bao/']);
      } catch (error) {
