...
export class HomeChitietnhiemvuComponent {
   ...
   async setForm() {
      if (this.maNv > 0) {
         await this.nhiemVuService.getById(this.maNv).then(async (data) => {
            this.nhiemVu = data;
            ...
            this.pdfSrc = environment.downloadLink + this.nhiemVu.fileNv + '?folder=Mission';
         });
      ...
      if (this.nvForm.form.valid) {
         ...
         try {
            if (file && file.files[0]) {
               let res = await this.fileService.uploadFile(file.files[0], 'Mission');
               nhiemVu.fileNv = res.fileName;
            }
