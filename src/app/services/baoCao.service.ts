...
export class baoCaoService {
    ...
    async add(BaoCao: BaoCao): Promise<any> {
      return await this.http.post(`${this.apiUrl}/api/Baocaos`, BaoCao, this.shareService.httpOptions).toPromise();
    }

    async update(BaoCao: BaoCao): Promise<any> {
      return await this.http.put<any>(`${this.apiUrl}/api/Baocaos/MaCv, MaSv, NamHoc, Dot, LanNop?MaCv=${BaoCao.maCv}&MaSv=${BaoCao.maSv}&NamHoc=${BaoCao.namHoc}&Dot=${BaoCao.dot}&LanNop=${BaoCao.lanNop}`, BaoCao,
       this.shareService.httpOptions).toPromise();
    }

    async delete(MaCv: string, MaSv: string, NamHoc: string, Dot: number, LanNop: number): Promise<any> {
      return await this.http.delete(`${this.apiUrl}/api/Baocaos/MaCv, MaSv, NamHoc, Dot, LanNop?MaCv=${MaCv}&MaSv=${MaSv}&NamHoc=${NamHoc}&Dot=${Dot}&LanNop=${LanNop}`,
      this.shareService.httpOptions).toPromise();
    }
}
