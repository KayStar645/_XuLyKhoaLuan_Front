...
export class chuyenNganhService {
  ...
  async getById(id: string): Promise<ChuyenNganh> {
    try {
      ...
      chuyenNganh =
        (await this.http
          .get<ChuyenNganh>(
            `${this.apiUrl}/api/Chuyennganhs/maCN?maCN=${id}`,
            this.shareService.httpOptions
          )
          .toPromise()) ?? (chuyenNganh as ChuyenNganh);
      return chuyenNganh;
    } catch (error) {
      throw error;
    }
  ...
  async update(chuyenNganh: ChuyenNganh): Promise<any> {
    return await this.http
      .put<any>(
        `${this.apiUrl}/api/Chuyennganhs/maCN?maCN=${chuyenNganh.maCn}`,
        chuyenNganh,
        this.shareService.httpOptions
      )
      .toPromise();
  ...
  async delete(maCN: string): Promise<any> {
    return await this.http
      .delete(
        `${this.apiUrl}/api/Chuyennganhs/maCN?maCN=${maCN}`,
        this.shareService.httpOptions
      )
      .toPromise();
  }

  async GetMaCnByTenAsync(tenCn: string): Promise<ChuyenNganh> {
    var chuyenNganh = new ChuyenNganh();
    return (
      (await this.http
        .get<ChuyenNganh>(
          `${this.apiUrl}/api/Chuyennganhs/tenCn?tenCn=${tenCn}`,
          this.shareService.httpOptions
        )
        .toPromise()) ?? chuyenNganh
    );
  }
}
