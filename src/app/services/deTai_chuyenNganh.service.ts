...
export class deTai_chuyenNganhService {
  ...
  async getAll(): Promise<DeTai_ChuyenNganh[]> {
    return (
      (await this.http
        .get<DeTai_ChuyenNganh[]>(
          `${this.apiUrl}/api/DetaiChuyennganh`,
  ...
  async GetChuyennganhByMaDT(maDt: string): Promise<ChuyenNganh[]> {
    return (
      (await this.http
        .get<ChuyenNganh[]>(
          `${this.apiUrl}/api/DetaiChuyennganh/maDT?maDT=${maDt}`,
          this.shareService.httpOptions
        )
        .toPromise()) ?? []
    );
