...
export class congViecService {
  ...
  async GetCongviecByMadt(maDt: string): Promise<CongViec[]> {
    return (
      (await this.http
        .get<CongViec[]>(
          `${this.apiUrl}/api/Congviecs/maDT?maDT=${maDt}`,
          this.shareService.httpOptions
        )
        .toPromise()) ?? []
    );
  ...
  async getById(maCV: string): Promise<CongViec> {
    try {
      ...
      response =
        (await this.http
          .get<CongViec>(
            `${this.apiUrl}/api/Congviecs/maCV?maCV=${maCV}`,
            this.shareService.httpOptions
          )
  ...
  async update(CongViec: CongViec): Promise<any> {
    return await this.http
      .put<any>(
        `${this.apiUrl}/api/Congviecs/maCV?maCV=${CongViec.maCv}`,
        CongViec,
        this.shareService.httpOptions
      )
      .toPromise();
  ...
  async isHaveCongViecForGroup(maNhom: string): Promise<boolean> {
    return (await this.getAll()).filter((cv) => cv.maNhom === maNhom).length > 0
      ? true
      : false;
  }
