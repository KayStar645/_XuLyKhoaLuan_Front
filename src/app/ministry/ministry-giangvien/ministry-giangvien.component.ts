...
export class MinistryGiangvienComponent implements OnInit {
   ...
   onShowFormUpdate() {
      ...
      if (Object.entries(this.DSGVComponent.lineGV).length > 0) {
         ...
         this.toastr.warning('Vui lòng chọn giảng viên để cập nhập thông tin', 'Thông báo !');
      }
   }
   ...
   handleToggleUpdate() {
      ...
      if (JSON.stringify(this.gvOldForm) !== JSON.stringify(this.gvForm.form.value)) {
   ...
   onNgaySinhInput(event: any) {
      ...
      let date2 = formValue.ngayNhanViec ? new Date(formValue.ngayNhanViec) : new Date();
      ...
      if (isSmaller) {
         ...
         ngaySinh.setValidators([
            this.shareService.customValidator('dateBirth18', / /, yearBetween >= 18 ? true : false),
            Validators.required,
         ]);
   ...
   resetLineActive() {
      if (document.querySelector('.table tr.br-line-hover')) {
         ...
         document.querySelector('.table tr.br-line-hover')?.classList.remove('br-line-hover');
      }
   }
      ...
      if (this.gvUpdateForm.valid) {
         if (JSON.stringify(this.gvOldForm) === JSON.stringify(this.gvForm.form.value)) {
            ...
            giangVien.init(
               ...
               this.gvUpdateForm.value['ngayNghi'] ? this.gvUpdateForm.value['ngayNghi'] : '',
            ...
            try {
               ...
               this.toastr.success('Cập nhập thông tin giảng viên viên thành công', 'Thông báo !');
               this.websocketService.sendForGiangVien(true);
            } catch {
